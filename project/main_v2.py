"""Main file of the CNN model, Train and Evaluate Validation"""
...
def train(model, dataloader, optimizer, critierion, metrics, params):
    ...
    with tqdm(total=training_step) as t:
        for i, (images_batch, labels_type_batch, labels_region_batch) in enumerate(dataloader):
            ...
            images_batch, labels_type_batch, labels_region_batch = Variable(images_batch), Variable(labels_type_batch), \
                                                                   Variable(labels_region_batch)
...
if __name__ == '__main__':
    ...
    num_class = 7
    num_region = 3
    model = net.resnet34(params, num_class, num_region).to(device)
