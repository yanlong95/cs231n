"""Main CNN architecture to be called in main.py and evaluate.py"""
...
class vgg16(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out1 = self.fc1(x_out)
        x_out1 = self.relu(x_out1)
        x_out1 = self.dropout(x_out1)
        x_out1 = self.fc2(x_out1)
        x_out1 = self.relu(x_out1)
        x_out1 = self.dropout(x_out1)
        x_out1 = self.fc3(x_out1)
        x_out1 = self.relu(x_out1)
        x_out1 = self.dropout(x_out1)
        x_out1 = F.log_softmax(x_out1, dim=1)

        x_out2 = self.fc1(x_out)
        x_out2 = self.relu(x_out2)
        x_out2 = self.dropout(x_out2)
        x_out2 = self.fc2(x_out2)
        x_out2 = self.relu(x_out2)
        x_out2 = self.dropout(x_out2)
        x_out2 = self.fc3(x_out2)
        x_out2 = self.relu(x_out2)
        x_out2 = self.dropout(x_out2)
...
class ResNet(nn.Module):
    ...
    def __init__(self, params, block, stages, num_classes=7, num_region=3, zero_init_residual=False, groups=1, width_per_group=64):
        ...
        self.fc1 = nn.Linear(512 * block.channel_expansion, 2 * 512 * block.channel_expansion)
        self.fc2 = nn.Linear(2 * 512 * block.channel_expansion, 2 * 512 * block.channel_expansion)
        self.fc3_class = nn.Linear(2 * 512 * block.channel_expansion, num_classes)
        self.fc3_region = nn.Linear(2 * 512 * block.channel_expansion, num_region)
        self.dropout = nn.Dropout(p=params.dropout_rate, inplace=True)
...
def resnet18(params, num_classes, num_region, pretrained=False, **kwargs):
    model = ResNet(params, Identity_block, [2, 2, 2, 2], num_classes=num_classes, num_region=num_region, **kwargs)
...
def resnet34(params, num_classes, num_region, pretrained=False, **kwargs):
    model = ResNet(params, Identity_block, [3, 4, 6, 3], num_classes=num_classes, num_region=num_region, **kwargs)
...
def resnet50(params, num_classes, num_region, pretrained=False, **kwargs):
    model = ResNet(params, Conv_block, [3, 4, 6, 3], num_classes=num_classes, num_region=num_region, **kwargs)
...
def resnet101(params, num_classes, num_region, pretrained=False, **kwargs):
    model = ResNet(params, Conv_block, [3, 4, 23, 3], num_classes=num_classes, num_region=num_region, **kwargs)
...
def resnet152(params, num_classes, num_region, pretrained=False, **kwargs):
    model = ResNet(params, Conv_block, [3, 8, 36, 3], num_classes=num_classes, num_region=num_region, **kwargs)
