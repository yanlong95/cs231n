"""Evaluates the model for the test set"""
...
def evaluate(model, critierion, dataloader, metrics, params):
    ...
    for idx, (data_batch, labels_type_batch, labels_region_batch) in enumerate(dataloader):
        ...
        if idx == 0:
          output_type_summ = output_type_batch
          labels_type_summ = labels_type_batch
        else:
          output_type_summ = np.concatenate((output_type_summ, output_type_batch),axis=0)
          labels_type_summ = np.concatenate((labels_type_summ, labels_type_batch),axis=0)
          #np.append(output_type_summ, output_type_batch)
          #np.append(labels_type_summ, labels_type_batch)
    ...
    print(output_type_summ.shape)
    class_report_f1 = net.classification_report(output_type_summ, labels_type_summ, output_dict=True)
    summary_batch_f1 = {}
    for key in class_report_f1:
            if len(key) == 1:
                summary_batch_f1['f1score-' + key] = np.float64(class_report_f1[key]['f1-score'])
                
    return metrics_mean, summary_batch_f1
...
if __name__ == '__main__':
    ...
    num_type = 7
    num_region = 3
    model = net.resnet50(params, num_type, num_region).cuda() if params.cuda else net.resnet50(params, num_type, num_region)
    ...
    # Evaluate
    test_metrics, summary_batch_f1 = evaluate(model, critierion, test_dl, metrics, params)
    visualize.plot_individual_label_f1score(summary_batch_f1,type='test')
