...
start_pt = 300
df_lat_lon = df_lat_lon[start_pt:(start_pt+500)]
...
with tqdm(total=len(df_lat_lon)) as t:
    for index, row in df_lat_lon.iterrows():
        ...
        sleep(5) # avoid too many requests exception
        ...
        # find building tag closest to the poi in way elements
        building_category = result.ways[0].tags.get('building')
        
        # append tag to dataframe
        row['tag'] = building_category
        
        # saving at every 100 iteration
        if index % 100 == 0 and index != 0:
            print("...saving checkpoint...")
            df_lat_lon.to_csv(filename[:-4] + '-category_labelled' + str(start_pt) + '.csv')
