...
df_osm_way.to_csv('centroid_csv/' + 'complete-' + filename[:-4] + '.csv')
...
for group_name, group_value in df_osm_grouped:
    ...
    csvname = filename[:-4] + '-' + str(group_name)
    print('...saving csv: ' + csvname + '...')
    polygon_centroid_df.to_csv('centroid_csv/' + csvname + '.csv')
    print()
